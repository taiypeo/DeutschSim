...
public class CustomGatePrompt extends JDialog {
    ...
    public CustomGatePrompt(final JFrame frame) {
        ...
        trig_selection = new AngleTypeSelection("What to represent arguments of " + "trigonometric functions in?");
        panel.add(trig_selection);

        option_pane = new JOptionPane(panel, JOptionPane.PLAIN_MESSAGE, JOptionPane.OK_OPTION, null,
                new Object[] { "Create Gate" });
    ...
    private void create_matrix_panel() {
        ...
        matrix = new TextEditor("Gate's unitary matrix representation", TextEditor.Type.MULTIPLE_LINE);
    ...
    private static class TextEditor extends JPanel {
        ...
        private void clear_background_text() {
            if (component.getText().equals(bg_text) && component.getForeground().equals(Color.GRAY)) {
    ...
    private class OKButtonListener implements PropertyChangeListener {
        ...
        public void propertyChange(PropertyChangeEvent ev) {
            ...
            if (isVisible() && ev.getSource() == option_pane
        ...
        private void create_rotation_gate() {
            String x_rot_text = x_rot.get_text(), y_rot_text = y_rot.get_text(), z_rot_text = z_rot.get_text();
            ...
            if (x_rot_complex.equals(y_rot_complex) && y_rot_complex.equals(z_rot_complex)
                    && z_rot_complex.equals(Complex.ZERO)) {
                dispose();
                return;
            }

            if (!Tools.equal(x_rot_complex.getImaginary(), 0.0) || !Tools.equal(y_rot_complex.getImaginary(), 0.0)
            ...
            final double x_rot_angle = x_rot_complex.getReal(), y_rot_angle = y_rot_complex.getReal(),
                    z_rot_angle = z_rot_complex.getReal();

            try {
                result_gate = new Gate(name, x_rot_angle, y_rot_angle, z_rot_angle, rot_selection.get_selected_type());
            } catch (RuntimeException ex) {
                Tools.error(frame, "A runtime exception has been caught:\n" + ex.getMessage());
                return;
            }
        ...
        private void create_matrix_gate() {
            ...
            List<String> matrix_text = Arrays.asList(matrix.get_text().replaceAll("\\s", "").split(","));
            ...
            try {
                ...
                for (int row = 0; row < matrix_dimension; row++)
                    for (int col = 0; col < matrix_dimension; col++) {
                        interpreter.set_lexical_analyzer(
                                new LexicalAnalyzer(matrix_text.get(row * matrix_dimension + col)));
